...
- Snapshot save/load with full daemon state persistence, resume from completed snapshots with extended maxSteps
- Auto-save snapshots at configurable interval ("Save Every" N steps)
- Screenshot capture (BMP via glReadPixels): manual "Snap" button + auto alongside snapshot saves
- Time-series visualization with persistent per-plot settings (Log Y, markers, line, marker size)
- Adaptive X-axis tick labels (auto-doubling interval to avoid overlap)
- Molecule Census UI window (sortable by size or count, persisted)
- Detailed log reporting (species by size, per-atom energy)
- UI Scale persisted across sessions
- Project structure: WORK/ (runtime), DOCS/ (documentation), source modules unchanged
